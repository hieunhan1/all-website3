...
$mail->AddAddress($to,'Dat hang Thien Phat Glass'); 
$mail->AddReplyTo($from,"admin"); 
$mail->WordWrap = 50; // set word wrap 
$mail->IsHTML(true); // send as HTML 
$mail->Subject = 'Đặt hàng '.$name_sp; 
$mail->HoTen = $name;
$mail->Email = $email;
$mail->LoiNhan = $content;
$mail->Body = "<h3>Chào ban quản trị website.</h3></br>Bạn &nbsp;<strong>{$name}</strong>&nbsp;đặt hàng <strong>{$name_sp}</strong>.<br /><br />
<br />Mã KH: <strong>{$id}</strong>
<br />Link SP: {$link_sp}
<br />Yêu cầu: {$content}
<br />Email: {$email}
<br />Phone: {$phone}
