...
<link type="icon/x-icon" href="images/logo_icon.ico" rel="shortcut icon" />';
	...
	function insert_khachhang($id,$name,$gender,$email,$dienthoai,$ip,$checksum,$thanhtoan_error,$ma_nganhang,$ma_sanpham){
		$date = date('Y-m-d H:i:s');
		$qr = "INSERT INTO thanhtoan_khachhang VALUES ('{$id}','{$name}','{$gender}','{$email}','{$dienthoai}','{$ip}','{$checksum}','{$thanhtoan_error}','{$ma_nganhang}','{$ma_sanpham}','0','{$date}',NULL ,'khachhang',NULL ,'0')";
		return mysql_query($qr);
	}
	...
	function update_khachhang($id,$stt,$status){
		...
		if(mysql_num_rows($qr)==1){
			...
			mysql_query("UPDATE thanhtoan_khachhang SET status='{$status}',thanhtoan_error='{$error}' WHERE `delete`=0 AND id='{$id}'");
		}
	}
