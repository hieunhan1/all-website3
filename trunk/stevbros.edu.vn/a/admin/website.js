...
function check_data(check_id, action_id){
	...
	if(check0.length < 1) input_disable(action_id);
	else if(check1.length < 1) input_disable(action_id);
	else if(check2.length < 1) input_disable(action_id);
...
check_data(['name','name_rewrite','url'],'btnSubmit');
