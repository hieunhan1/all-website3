...
if($id == 0){ //create
	...
	//hidden field id
	$form->getProperties(NULL, 'id', 2, NULL, $id, 3);
	$user_login = $form->DisplayProperties();
...
$style1 = '&nbsp;';
$style2 = '&nbsp;---';
$str_query = array("SELECT id, name FROM menu ", " WHERE `delete`=0 AND parent_id= ", " ORDER BY `order` ASC ");
...
//info
if(@$_POST['info']) $value = $_POST['info']; else $value = $detail['info'];
$form->getProperties('Thông tin', 'info', 3, 'textarea', $value, 1, '');
echo $form->DisplayProperties();
