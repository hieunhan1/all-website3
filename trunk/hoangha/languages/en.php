<?php
define(home_page,'home');
define(const_home_info,'New Information');
define(const_tin_khac, 'Other post');

define(const_thong_tin, 'Contact Information');
define(const_date_update, 'Update ');
define(const_txt_search, 'enter keywords..');

define(const_contact_name, 'Name');
define(const_contact_phone, 'Telephone');
define(const_contact_diachi, 'Address');
define(const_contact_message, 'Message');
define(const_contact_sent, 'SEND');

define(const_contact_mathang, 'Items');
define(const_contact_date, 'Date');
define(const_contact_from, 'From');
define(const_contact_to, 'To');
define(const_contact_soluong, 'The number of');
define(const_contact_nhietdo, 'Temperature');
define(const_contact_yeucau, 'Request');

//tracking & tracktrace
define(const_contact_ma, 'Code');
define(const_contact_dinhvi_buupham, 'Parcels located');

?>