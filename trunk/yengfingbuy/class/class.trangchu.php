...
<link type="icon/x-icon" href="images/logo_icon.ico" rel="shortcut icon" />';
	...
	function phantrang($link,$page_number,$max_results,$select,$table,$where,$keyword=NULL,$catalog=NULL,$order=NULL){
		...
		if($total_pages > 1){
			for($i = $page_number - 5; $i <= $page_number + 5 & $i <= $total_pages; $i++){
				if($page_number == $i){
					$str .= "<a href='{$link}/{$i}/' style='font-weight:bold; background-color:#000; color:#FFF'>{$i}</a>";
				} elseif($i>0) {
	...
	/*home*/
	function danhmuc_sp($lang){
		$qr = $this->menu_type(3,0,$lang);
		$row = mysql_fetch_array($qr);
		return $row['id'];
	}
	function home_about($id){
		$qr = "SELECT name,url_hinh,metaDescription FROM info WHERE `delete`=0 AND status=1 AND menu_id LIKE '%,{$id},%' LIMIT 1";
		return mysql_query($qr);
	}
	...
	function tuyendung_detail($alias){
		$qr = "SELECT name,name_rewrite,url_hinh,metaDescription,content,metaKeyword,soluong_tuyen,noilamviec,date_update FROM tuyendung WHERE `delete`=0 AND status=1 AND name_rewrite='{$alias}'";
		return mysql_query($qr);
	}
