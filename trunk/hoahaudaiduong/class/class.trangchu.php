...
<link type="icon/x-icon" href="images/logo_icon.ico" rel="shortcut icon" />';
	...
	function info_noibat($lang){
		$qr = "SELECT name,name_rewrite,url_hinh,metaDescription,menu_id FROM info WHERE `delete`=0 AND status=1 AND `other`=1 AND lang='{$lang}' ORDER BY date_update DESC LIMIT 7";
		return mysql_query($qr);
	}
	...
	function home_thisinh($lang){
		$qr = "SELECT name,name_rewrite,url_hinh,sbd,menu_id FROM thisinh WHERE `delete`=0 AND status=1 AND other=1 AND lang='{$lang}' ORDER BY date_update DESC LIMIT 44";
		return mysql_query($qr);
	}
