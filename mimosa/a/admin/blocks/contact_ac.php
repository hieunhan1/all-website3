...
echo $form->DisplayProperties();
//tentranh
if(@$_POST['tentranh']) $value = $_POST['tentranh']; else $value = $detail['tentranh'];
$form->getProperties("Tên tranh", 'tentranh', 1, 'input_medium', $value, 150);
echo $form->DisplayProperties();
//linktranh
if(@$_POST['linktranh']) $value = $_POST['linktranh']; else $value = $detail['linktranh'];
$form->getProperties("Link tranh", 'linktranh', 1, 'input_medium', $value, 150);
echo $form->DisplayProperties();
//kichthuoc
if(@$_POST['kichthuoc']) $value = $_POST['kichthuoc']; else $value = $detail['kichthuoc'];
$form->getProperties("Kích thước", 'kichthuoc', 1, 'input_medium', $value, 150);
echo $form->DisplayProperties();
//tranhmythuat
$value = array(1 => 'Có', 0 => 'Không');
if($_POST['tranhmythuat'] != '') $check = $_POST['tranhmythuat'];
else if($detail['tranhmythuat'] != '') $check = $detail['tranhmythuat'];
else $check = 0; //giá trị mặc định
$form->getProperties('Tranh mỹ thuật', 'tranhmythuat', 5, $check, $value, ' &nbsp; ');
echo $form->DisplayProperties();
//message
$orther = "
<script>
