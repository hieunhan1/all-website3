...
<link type="icon/x-icon" href="images/logo_icon.ico" rel="shortcut icon" />';
	...
	/*home*/
	function home_info($idMenu){
		$qr	= "SELECT name,content FROM web_info WHERE `delete`=0 AND status=1 AND menu_id LIKE '%,{$idMenu},%' LIMIT 1";
		return mysql_query($qr);
	}
	function home_info_item($limt){
		$qr	= "SELECT name,name_rewrite,url_hinh,metaDescription,menu_id FROM web_info WHERE `delete`=0 AND status=1 AND other=1 LIMIT {$limt}";
		return mysql_query($qr);
	}
