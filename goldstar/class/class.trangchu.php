...
<link type="icon/x-icon" href="images/logo_icon.ico" rel="shortcut icon" />';
	...
	function home_about($idMenu){
		$qr = "SELECT name,name_rewrite,url_hinh,metaDescription,menu_id FROM info WHERE `delete`=0 AND status=1 AND menu_id LIKE '%,{$idMenu},%'";
		return mysql_query($qr);
	}
	function home_list($lang){
		$qr = "SELECT id,name,url,url_hinh,title FROM menu WHERE `delete`=0 AND status=1 AND other=1 AND lang='{$lang}' ORDER BY `order`";
		return mysql_query($qr);
	}
	function home_list_product($idMenu,$limit=NULL,$id=NULL){
		if($limit!=NULL) $limit = "LIMIT {$limit}"; else $limit = '';
		if($id==NULL) $id = ''; else $id = "AND id<>'{$id}'";
		$qr = "SELECT name,name_rewrite,price,price_km,url_hinh,menu_id FROM products WHERE `delete`=0 AND status=1 AND other=1 AND menu_id LIKE '%,{$idMenu},%' {$id} ORDER BY date_update DESC {$limit}";
		return mysql_query($qr);
	}
	
	/*other*/
	function info_left($idMenu,$limit=NULL){
		if($limit!=NULL) $limit = "LIMIT {$limit}"; else $limit = '';
		$qr = "SELECT name,name_rewrite,menu_id FROM info WHERE `delete`=0 AND status=1 AND menu_id LIKE '%,{$idMenu},%' ORDER BY date_update {$limit}";
		return mysql_query($qr);
	}
	function info_detail($alias){
		$qr = "SELECT id,name,name_rewrite,url_hinh,metaDescription,content,metaKeyword,date_update FROM info WHERE `delete`=0 AND status=1 AND name_rewrite='{$alias}'";
		return mysql_query($qr);
	}
	function info_product($alias){
