...
<link type="icon/x-icon" href="images/logo_icon.ico" rel="shortcut icon" />';
	...
	function phantrang($link,$page_number,$max_results,$select,$table,$where,$keyword=NULL,$catalog=NULL,$order=NULL){
		...
		if($total_pages > 1){
			...
			for($i = $page_number - 5; $i <= $page_number + 5 & $i <= $total_pages; $i++){
				if($page_number == $i){
					$str .= "<div class='item_page'><a href='{$link}{$i}/' style='color:#FFF; font-weight:bold; background:url(images/phantrang-ac.png)'>{$i}</a></div>";
				} elseif($i>0) {
					$str .= "<div class='item_page'><a href='{$link}{$i}/'>{$i}</a></div>";
				}
			}
	...
	/*home*/
	function home_sanpham(){
		$qr = "SELECT name,name_rewrite,url_hinh,menu_id FROM products WHERE `delete`=0 AND status=1 AND `other`=1 ORDER BY date_update DESC LIMIT ".max_product_home;
		return mysql_query($qr);
	}
	...
	function other_product($idMenu,$id){
		$qr = "SELECT name,name_rewrite,url_hinh,menu_id FROM products WHERE `delete`=0 AND status=1 AND id<>'{$id}' AND menu_id LIKE '%,{$idMenu},%' ORDER BY date_update DESC LIMIT 3";
		return mysql_query($qr);
	}
	function product_all($idMenu){
		$qr = "SELECT name,name_rewrite,url_hinh,menu_id FROM products WHERE `delete`=0 AND status=1 AND menu_id LIKE '%,{$idMenu},%' ORDER BY date_update DESC, menu_id ASC";
		return mysql_query($qr);
	}
