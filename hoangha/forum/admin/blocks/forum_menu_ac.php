...
$style1 = '&nbsp;';
$style2 = '&nbsp;--- ';
$str_query = array("SELECT id, name FROM forum_menu ", " WHERE `delete`=0 AND lang='".$_SESSION['language']."' AND parent_id= ", " ORDER BY `order` ");
...
//Vị trí menu
$value = array();
$qr = mysql_query("SELECT * FROM forum_menu_position WHERE `delete`=0 ");
...
//Loại menu
$value = array();
$qr = mysql_query("SELECT * FROM forum_menu_type WHERE `delete`=0 ");
while($row_temp = mysql_fetch_array($qr)){
