...
$style1 = '&nbsp;';
$style2 = '&nbsp;--- ';
$str_query = array("SELECT id, name FROM forum_menu ", " WHERE `delete`=0 AND parent_id= ", " ORDER BY `order` ");
...
$form->getProperties('Danh mục', 'menu_id', 6, 'input_large', $values, $check);
echo $form->DisplayProperties();

//Mô tả name
if(@$_POST['name']) $value = $_POST['name']; else $value = $detail['name'];
$form->getProperties('Tiêu đề', 'name', 1, 'input_medium', $value, 250);
echo $form->DisplayProperties();
...
$form->getProperties('Cho trả lời', 'other', 5, $check, $value, ' &nbsp; ');
echo $form->DisplayProperties();
