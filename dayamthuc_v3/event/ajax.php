...
if(isset($_POST['thongtincanhan'])){
	...
	if($name!='' && $gender!='' && $phone!='' && $email!='' && $address!=''){
		...
		$sql = "INSERT INTO `web_event` (`name`,`gender`,`phone`,`email`,`address`,`ip_address`,`date_start`,`date_create`) VALUES ('{$name}','{$gender}','{$phone}','{$email}','{$address}','{$ip_address}','{$date_start}','{$date_current}')";
		if(mysql_query($sql)){
			echo 1;
			return true;
		}else{
